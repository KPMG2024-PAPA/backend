...
def encode_image(image_input: Union[str, bytes]):
    """Encode an image to base64, handling both file paths and bytes."""
    if isinstance(image_input, str):  # If image_input is a file path
        with open(image_input, "rb") as image_file:
            return base64.b64encode(image_file.read()).decode('utf-8')
    elif isinstance(image_input, bytes):  # If image_input is already in bytes
        return base64.b64encode(image_input).decode('utf-8')
    else:
        raise ValueError("Unsupported image input type")


def submit_spec(summary, image_path):
    """발명 요약과 도면을 첨부하여 특허 초안을 작성해준다."""
    ...
    response = client.chat.completions.create(
        ...
        [발명을 실시하기 위한 구체적인 내용] \n 중요한 권리 부분으로 보호받고자하는 기술 내용 설명하는 부분입니다. 발명의 구성요소를 도면과 관련하여 번호를 붙여가며 각각의 역할을 자세히 서술하세요. [예]상기 볼트(52)에 나사 결합되고 무게추(53)를 수납할 수 있도록 추 공간부(54)를 형성한 볼트캡(55)을 포함하여 구성된다.
        [특허청구의 범위] \n 보호받고자 하는 기술내용의 요약을 포함하는 청구항을 구체적으로 작성하세요. 각 청구항은 항마다 행을 바꾸어 기재하고, 기재하는 순서에 번호를 붙이세요.
    ...
    제시되는 도면 이미지와 발명 요약을 기반으로 특허 명세서를 작성해주세요. 특허 명세서의 형식은 <특허 명세서의 구성>을 따라서 작성해주세요. 평서문 해라체(예. ~ 한다. ~이다.) 를 사용하여 작성합니다.
...
# sum = input('발명 요약 : ')
# image_path = "./gpt/test_image.jpg"
# response_text = submit_spec(sum, image_path)
# print(response_text)
