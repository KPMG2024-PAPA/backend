...
# ChromaDB 클라이언트 설정
chromadb_client = chromadb.HttpClient(host="localhost", port=5000)
korean_patents = chromadb_client.get_collection(name="korean_patents")
foreign_patents = chromadb_client.get_collection(name="foriegn_patents")
    ...
    for id, distance, IPC_code, metadata in zip(ids, distances, IPC_codes, metadatas):
        ...
        if any(ipc in unique_IPCs for ipc in IPC_code):
            filtered_results.append({
                ...
                "요약": metadata.get('요약'),
                "출원번호": metadata.get('출원번호'),
                "발명의명칭": metadata.get("발명의명칭")
            })
...
def foreign_top_similarity(query: str, n_results: int,  IPCs: List[str] = []):
    ...
    separated_IPC_codes = [[ipc.strip() for ipc in str(IPC_code).split(',')] for IPC_code in IPC_codes]
    # 필터링된 결과를 저장할 리스트 초기화
    filtered_results = []

    # metadatas를 순회하며 필터링 조건에 맞는 문서 선택
    for id, distance, IPC_code, metadata in zip(ids, distances, separated_IPC_codes, metadatas):
        if metadata == None:
            continue
        # IPC_code 리스트와 unique_IPCs 리스트의 교집합이 있는지 확인
        if any(ipc in unique_IPCs for ipc in IPC_code):
